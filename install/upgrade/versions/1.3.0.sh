...
# DevIT Control Panel upgrade script for target version 1.3.0
...
if [ ! -e "/etc/cron.d/DevIT-proc" ]; then
...
# Set LOGIN_STYLE variable in DevIT.conf
echo "[ * ] Updating configuration file: DevIT.conf..."
$BIN/v-change-sys-config-value "LOGIN_STYLE" "default"

# Remove old lanugage files.
if [ -e $DevIT/web/inc/i18n/en.php ]; then
	echo "[ * ] Performing language library clean-up..."
	rm -fr $DevIT/web/inc/i18n
fi
