...
# DevIT Control Panel upgrade script for target version 1.8.0
...
if ! grep -qw "^[DevIT_openssl_sect]$" /etc/ssl/openssl.cnf 2> /dev/null; then
...
if [ "$WEB_SYSTEM" = "nginx" ] || [ "$PROXY_SYSTEM" = "nginx" ]; then
	...
	if ! grep -q "quic_bpf" /etc/nginx/nginx.conf && ! grep -q "spdy_headers_comp" /etc/nginx/nginx.conf; then
		...
		curl -fsLm5 --retry 2 https://raw.githubusercontent.com/DevITcp/DevITcp/b2ad1549a21655837056e4b7883970d51a4b324f/install/deb/nginx/nginx.conf \
		...
		curl -fsLm5 --retry 2 https://raw.githubusercontent.com/DevITcp/DevITcp/015b20ae1ffb82faaf58b41a5dc9ad1b078b785f/install/deb/nginx/nginx.conf \
		...
		curl -fsLm5 --retry 2 https://raw.githubusercontent.com/DevITcp/DevITcp/555f89243e54e02458586ae4f7999458cc9d33e9/install/deb/nginx/nginx.conf \
		...
		if [ "$nginx_conf_compare" != "same" ]; then
			...
			add_upgrade_message "Manual Action Required [IMPORTANT]\n\nTo enable the \"Enhanced and Optimized TLS\" feature, we must update the NGINX configuration file (/etc/nginx/nginx.conf).\n\nBut for unknown reason or you edited it, may not be fully apply all the changes in this upgrade.\n\nPlease follow the default configuration file to sync it:\n$DevIT_INSTALL_DIR/nginx/nginx.conf\n\nBacked up configuration file:\n$DevIT_BACKUP/conf/nginx/nginx.conf\n\nLearn more:\nhttps://github.com/DevITcp/DevITcp/pull/3555\n\n"
			"$BIN"/v-add-user-notification admin "IMPORTANT: Manual Action Required" '<p>To enable the "Enhanced and Optimized TLS" feature, we must update the NGINX configuration file at <code>/etc/nginx/nginx.conf</code>.</p><p>But for unknown reason or you edited it, may not be fully apply all the changes in this upgrade.</p><p>Please follow the default configuration file to sync it:<br><code>'"$DevIT_INSTALL_DIR"'/nginx/nginx.conf</code></p><p>Backed up configuration file:<br><code>'"$DevIT_BACKUP"'/conf/nginx/nginx.conf</code></p><p>Visit PR <a href="https://github.com/DevITcp/DevITcp/pull/3555" target="_blank">#3555</a> on GitHub to learn more.</p>'

			sed -i "s/""$(grep -m 1 "IMPORTANT: Manual Action Required" "$DevIT"/data/users/admin/notifications.conf | awk '{print $1}')""/NID='1'/" "$DevIT"/data/users/admin/notifications.conf
		...
		add_upgrade_message "About TLS 1.3 0-RTT anti-replay for NGINX\n\nIf you use custom templates, please update them (*.stpl) to apply this protection.\n\nFollow the usage or other default templates:\n/etc/nginx/conf.d/0rtt-anti-replay.conf\n\nLearn more:\nhttps://github.com/DevITcp/DevITcp/pull/3692"
		"$BIN"/v-add-user-notification admin "About TLS 1.3 0-RTT anti-replay for NGINX" '<p>If you use custom templates, please update them (*.stpl) to apply this protection.</p><p>Follow the usage or other default templates:<br><code>/etc/nginx/conf.d/0rtt-anti-replay.conf</code></p><p>Visit PR <a href="https://github.com/DevITcp/DevITcp/pull/3692" target="_blank">#3692</a> on GitHub to learn more.</p>'

		if grep -qw "IMPORTANT: Manual Action Required" "$DevIT"/data/users/admin/notifications.conf 2> /dev/null; then
			sed -i "s/""$(grep -m 1 "About TLS 1.3 0-RTT anti-replay for NGINX" "$DevIT"/data/users/admin/notifications.conf | awk '{print $1}')""/NID='2'/" "$DevIT"/data/users/admin/notifications.conf
		else
			sed -i "s/""$(grep -m 1 "About TLS 1.3 0-RTT anti-replay for NGINX" "$DevIT"/data/users/admin/notifications.conf | awk '{print $1}')""/NID='1'/" "$DevIT"/data/users/admin/notifications.conf
		fi
		...
		cp -f "$DevIT_INSTALL_DIR"/nginx/phpmyadmin.inc "$DevIT_INSTALL_DIR"/nginx/phppgadmin.inc "$DevIT_INSTALL_DIR"/nginx/status.conf /etc/nginx/conf.d
...
# shellcheck source=/usr/local/DevIT/func/domain.sh
source $DevIT/func/domain.sh

if [ "$WEB_SYSTEM" = "nginx" ]; then
	while IFS= read -r IP; do
		ip_conf="/etc/nginx/conf.d/$IP.conf"
		cp -f "$DevIT_INSTALL_DIR"/nginx/unassigned.inc "$ip_conf"
		sed -i "s/directIP/$IP/g" "$ip_conf"
		process_http2_directive "$ip_conf"
	done < <(ls "$DevIT"/data/ips/ 2> /dev/null)
...
if [ "$MAIL_SYSTEM" = "exim4" ]; then
	...
	if version_ge "$exim_version" "4.95"; then
		if ! grep -q 'SRS_SECRET' /etc/exim4/exim4.conf.template; then
			...
			patch /etc/exim4/exim4.conf.template.staging $DevIT/install/upgrade/patch/3661-exim-srs-support.patch 2>&1
			exim -C /etc/exim4/exim4.conf.template.staging 2>&1
			if [ "$?" -ne 0 ]; then
				add_upgrade_message "Unable to successfully aply the SRS update patch for Exim.\n If you use SMTP relay with the SRS feature use the exim config found in /usr/local/DevIT/install/deb/exim/exim4.conf.4.95.template"
				"$BIN"/v-add-user-notification admin "Unable to apply patch to Exim config" 'Unable to successfully apply the SRS update patch for Exim.<br /> If you use SMTP relay with the SRS feature use the exim config found in /usr/local/DevIT/install/deb/exim/exim4.conf.4.95.template'
				if grep -qw "IMPORTANT: Manual Action Required" "$DevIT"/data/users/admin/notifications.conf 2> /dev/null; then
					sed -i "s/""$(grep -m 1 "Unable to apply patch to Exim config" "$DevIT"/data/users/admin/notifications.conf | awk '{print $1}')""/NID='3'/" "$DevIT"/data/users/admin/notifications.conf
				else
					sed -i "s/""$(grep -m 1 "Unable to apply patch to Exim config" "$DevIT"/data/users/admin/notifications.conf | awk '{print $1}')""/NID='2'/" "$DevIT"/data/users/admin/notifications.conf
				fi
