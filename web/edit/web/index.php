...
if (!empty($v_ssl)) {
	exec(
		HESTIA_CMD . "v-list-web-domain-ssl " . $user . " " . escapeshellarg($v_domain) . " json",
		$output,
		$return_var,
	);
...
if (!empty($_POST["save"])) {
	...
	if ($v_ip != $_POST["v_ip"] && empty($_SESSION["error_msg"])) {
		exec(
			HESTIA_CMD . "v-list-dns-domain " . $user . " " . escapeshellarg($v_domain) . " json",
			$output,
			$return_var,
		);
		unset($output);
	...
	if ($v_ip != $_POST["v_ip"] && empty($_SESSION["error_msg"])) {
		exec(
			HESTIA_CMD . "v-list-mail-domain " . $user . " " . escapeshellarg($v_domain) . " json",
			$output,
			$return_var,
		);
		unset($output);
		if ($return_var == 0) {
			exec(
				HESTIA_CMD . "v-rebuild-mail-domain " . $user . " " . escapeshellarg($v_domain),
				$output,
				$return_var,
			);
			check_return_code($return_var, $output);
		...
		if ($v_template != $_POST["v_template"] && empty($_SESSION["error_msg"])) {
			exec(
				HESTIA_CMD .
					...
					escapeshellarg($v_domain) .
					" " .
					escapeshellarg($_POST["v_template"]) .
					" 'no'",
				$output,
				$return_var,
			);
		...
		if (
			...
			exec(
				HESTIA_CMD .
					...
					escapeshellarg($v_domain) .
					" " .
					escapeshellarg($v_backend_template),
				$output,
				$return_var,
			);
			check_return_code($return_var, $output);
			...
			if ($_POST["v_nginx_cache_check"] == "on") {
				...
				exec(
					HESTIA_CMD .
						...
						escapeshellarg($v_domain) .
						" " .
						escapeshellarg($_POST["v_nginx_cache_duration"]),
					$output,
					$return_var,
				);
				check_return_code($return_var, $output);
			...
			if ($v_proxy_template != $_POST["v_proxy_template"] || $v_proxy_ext != $ext) {
				...
				exec(
					HESTIA_CMD .
						...
						escapeshellarg($v_domain) .
						" " .
						escapeshellarg($v_proxy_template) .
						" " .
						escapeshellarg($ext) .
						" 'no'",
					$output,
	...
	if (empty($_SESSION["error_msg"])) {
		...
		foreach ($result as $alias) {
			if (empty($_SESSION["error_msg"]) && !empty($alias)) {
				...
				if (empty($_SESSION["error_msg"])) {
					exec(
						HESTIA_CMD . "v-list-dns-domain " . $user . " " . escapeshellarg($v_domain),
						$output,
						$return_var,
					);
					unset($output);
					...
					if ($return_var == 0) {
						exec(
							HESTIA_CMD .
								...
								escapeshellarg($alias) .
								" " .
								escapeshellarg($v_newip_public ?: $v_ip_public) .
								" no",
							$output,
							$return_var,
						);
	...
	if (
		...
		exec(
			HESTIA_CMD . "v-add-web-domain-ssl-force " . $user . " " . escapeshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (!empty($_POST["v_ssl_hsts"]) && !empty($_POST["v_ssl"]) && empty($_SESSION["error_msg"])) {
		exec(
			HESTIA_CMD . "v-add-web-domain-ssl-hsts " . $user . " " . escapeshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (
		...
		exec(
			HESTIA_CMD . "v-delete-web-domain-ssl-force " . $user . " " . escapeshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if ($v_ssl_hsts == "yes" && empty($_POST["v_ssl_hsts"]) && empty($_SESSION["error_msg"])) {
		exec(
			HESTIA_CMD . "v-delete-web-domain-ssl-hsts " . $user . " " . escapeshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (!empty($v_stats) && $_POST["v_stats"] == "none" && empty($_SESSION["error_msg"])) {
		exec(
			HESTIA_CMD . "v-delete-web-domain-stats " . $user . " " . escapeshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (!empty($_POST["v_ftp_user"])) {
		...
		foreach ($_POST["v_ftp_user"] as $i => $v_ftp_user_data) {
			...
			if (!empty($_POST["v_ftp"])) {
				...
				$v_ftp_username_for_emailing = $v_ftp_user_data["v_ftp_user"];
				$v_ftp_username = $user_plain . "_" . $v_ftp_user_data["v_ftp_user"];
				// preg_replace("/^".$user."_/", "", $v_ftp_user_data['v_ftp_user']);
	...
	if (!empty($v_redirect) && empty($_POST["v-redirect-checkbox"])) {
		exec(
			HESTIA_CMD . "v-delete-web-domain-redirect " . $user . " " . escapeshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
			...
			if ($_POST["v-redirect"] == "custom" && empty($_POST["v-redirect-custom"])) {
				...
				exec(
					HESTIA_CMD .
						...
						escapeshellarg($v_domain) .
						" " .
						escapeshellarg($_POST["v-redirect"]) .
						" " .
						escapeshellarg($_POST["v-redirect-code"]),
					$output,
					$return_var,
				);
				check_return_code($return_var, $output);
