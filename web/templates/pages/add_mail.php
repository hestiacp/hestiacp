        ...
        <div class="form-container">
            ...
            <?php if ($_SESSION["role"] == "admin" && $accept !== "true") { ?>
                <div class="alert alert-danger" role="alert">
                    ...
                    ?>
                    <p>
                        <?= htmlify_trans($warning, '</a>', '<a href="/add/user/">'); ?>
                    </p>
                </div>
            <?php } ?>
            <?php if ($_SESSION["role"] == "admin" && empty($accept)) { ?>
                <div class="u-side-by-side u-mt20">
                    ...
                    <a href="/add/mail/?accept=true" class="button button-danger u-width-full u-ml10">
                        <?= _("Continue") ?>
                    </a>
                </div>
            <?php } ?>
            <?php if (($_SESSION["role"] == "admin" && $accept === "true") || $_SESSION["role"] !== "admin") { ?>
                    ...
                    <div class="u-mb20">
                        ...
                        <select class="form-select" name="v_webmail" id="v_webmail" tabindex="6">
                            <?php foreach ($webmail_clients as $client) : ?>
                                <option
                                    value="<?= htmlentities($client) ?>"
                                    <?= ($v_webmail == $client) ? 'selected' : '' ?>>
                                    <?= htmlentities(ucfirst($client)) ?>
                                </option>
                            <?php endforeach; ?>
                            <option value=""
                                <?php if (empty($v_webmail) || $v_webmail == 'disabled') : ?>
                                selected
                                <?php endif; ?>>
                                <?= _("Disabled") ?>
                            </option>
                ...
                <?php if (!empty($_SESSION["ANTISPAM_SYSTEM"])) { ?>
                    <div class="form-check u-mb10">
                        <input
                            ...
                            name="v_antispam"
                            id="v_antispam"
                            <?= (empty($v_antispam) || $v_antispam == "yes") ? "checked" : "" ?>>
                    ...
                    <div class="form-check u-mb10">
                        <input
                            ...
                            name="v_reject"
                            id="v_reject"
                            <?= (empty($v_reject) || $v_reject == 'yes') ? 'checked' : '' ?>>
                        <label for="v_reject">
                            <?= _("Reject Spam") ?>
                        </label>
                    </div>
                ...
                <?php if (!empty($_SESSION['ANTIVIRUS_SYSTEM'])) { ?>
                    <div class="form-check u-mb10">
                        <input
                            ...
                            name="v_antivirus"
                            id="v_antivirus"
                            <?= (empty($v_antivirus) || $v_antivirus == 'yes') ? 'checked' : '' ?>>
                ...
                <div class="form-check u-mb10">
                    <input
                        ...
                        name="v_dkim"
                        id="v_dkim"
                        <?= (isset($v_dkim) && $v_dkim == 'yes') ? 'checked' : '' ?>>
                    <label for="v_dkim">
                        <?= _("DKIM Support") ?>
                    </label>
                </div>
