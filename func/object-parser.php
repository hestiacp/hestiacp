...
function parseObjectKvLine(string $unparsed, array $reserved, array $exceptions): array {
	$result = [];
	...
	while ($unparsed !== "") {
		...
		$keyNameExtracted = preg_match("/^([a-zA-Z][a-zA-Z0-9_]*)=/", $unparsed, $matches);
		...
		$skipReserved =
			...
			(in_array($keyName, $reserved, true) || strpos($keyName, "BASH_FUNC_") === 0);
		...
		while (true) {
			...
			$match = preg_match('/\s|\'|\\\\/u', $unparsed, $matches, PREG_OFFSET_CAPTURE);
			...
			$keyValue .= substr($unparsed, 0, $pos);
			$unparsed = substr($unparsed, $pos + strlen($matchedChar));

			if ($matchedChar === "'") {
				if ($unparsed === "") {
					objectParserFail(
						"Invalid key value format. No closing quote for key: " . $keyName,
					);
				}
			...
			if ($matchedChar === "\\") {
				...
				$nextChar = mb_substr($unparsed, 0, 1, "UTF-8");

				$keyValue .= $nextChar;
				$unparsed = substr($unparsed, strlen($nextChar));
		...
		if (array_key_exists($keyName, $result)) {
			$message = "Warning: Duplicate key name: " . $keyName . ". ";
...
function parseObjectKvFile(string $file, array $reserved, array $exceptions): array {
	...
	foreach ($lines as $line) {
		$records[] = parseObjectKvLine($line, $reserved, $exceptions);
	}
...
function objectParserEncodeJson(array $data): string {
	try {
		...
		objectParserFail("Could not encode JSON: " . $exception->getMessage());
	}
}
...
if (PHP_SAPI === "cli" && realpath($_SERVER["SCRIPT_FILENAME"] ?? "") === realpath(__FILE__)) {
	if ($argc < 5 || $argv[1] !== "line-shell") {
		objectParserFail(
			"Usage: object-parser.php line-shell " . '"KEY=value ..." RESERVED_KEYS EXCEPTIONS',
		);
	}

	$result = parseObjectKvLine($argv[2], parseObjectKvList($argv[3]), parseObjectKvList($argv[4]));
