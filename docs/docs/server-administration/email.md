...
By default, emails generated by Hestia (notifications, password resets, update logs, etc.) are sent through the internal mail server. If you prefer, you can configure Hestia to send emails through an SMTP account.

You can easily configure Server SMTP directly from the HestiaCP Web interface:

1. Log in to HestiaCP as an administrator.
2. Navigate to **Server** (the gear icon) and click **Configure**.
3. Expand the **Notifications** section.
4. Toggle **Use Server SMTP** and fill in your SMTP parameters:
   - Host (e.g. `smtp.example.com`)
   - Port (e.g. `25`, `465` or `587`)
   - Security (e.g. `STARTTLS`)
   - Username
   - Password
   - Sender Email Address (i.e. the sender address).
5. (Optional) Click **Test Email** to verify your connection immediately.
6. Click **Save**.

Alternatively, you can configure it via SSH using the included setup script:
