...
if [ "$nginx" = 'yes' ]; then
    echo "[ * ] NGINX"
    echo "deb [arch=arm64] https://nginx.org/packages/mainline/$VERSION/ $codename nginx" > $apt/nginx.list
...
if [ "$apache" = 'yes' ]; then
    echo "[ * ] Apache2"
    echo "deb [arch=arm64] http://ppa.launchpad.net/ondrej/apache2/ubuntu $codename main" > $apt/apache2.list
fi

# Installing MariaDB repo
if [ "$mysql" = 'yes' ]; then
    echo "[ * ] MariaDB"
    echo "deb [arch=arm64] https://mirror.mva-n.net/mariadb/repo/$mariadb_v/$VERSION $codename main" > $apt/mariadb.list
    apt-key adv --fetch-keys 'https://mariadb.org/mariadb_release_signing_key.asc' > /dev/null 2>&1
fi

# Installing HestiaCP repo
echo "[ * ] Hestia Control Panel"
echo "deb [arch=arm64] https://$RHOST/ $codename main" > $apt/hestia.list
apt-key adv --keyserver keyserver.ubuntu.com --recv-keys A189E93654F0B0E5 > /dev/null 2>&1

# Installing PostgreSQL repo
if [ "$postgresql" = 'yes' ]; then
    echo "[ * ] PostgreSQL"
    echo "deb [arch=arm64] https://apt.postgresql.org/pub/repos/apt/ $codename-pgdg main" > $apt/postgresql.list
