		...
		<div class="toolbar-right">
			<div class="toolbar-sorting">
				...
				<ul class="toolbar-sorting-menu animate__animated animate__fadeIn u-hidden">
					<li entity="sort-date" sort_as_int="1"><span class="name <?php if ($_SESSION["userSortOrder"] === "date") {
     	echo "active";
     } ?>"><?= _("Date") ?> <i class="fas fa-arrow-down-a-z"></i></span><span class="up"><i class="fas fa-arrow-up-a-z"></i></span></li>
					<li entity="sort-name"><span class="name <?php if ($_SESSION["userSortOrder"] === "name") {
     	echo "active";
     } ?>"><?= _("Name") ?> <i class="fas fa-arrow-down-a-z"></i></span><span class="up"><i class="fas fa-arrow-up-a-z"></i></span></li>
				</ul>
...
<div class="container units">
	...
	<?php foreach ($data as $key => $value) {
 	...
 	?>
		<div class="l-unit <?php if ($status == "suspended") {
  	echo "l-unit--suspended";
  } ?> animate__animated animate__fadeIn" id="web-unit-<?= $i ?>" uniq-id="<?= $uniq_id ?>" sort-date="<?= strtotime($value["DATE"] . " " . $value["TIME"]) ?>" sort-name="<?= $value[
	"RESULT"
] ?>" sort-type="<?= _($object) ?>" sort-owner="<?= $value["USER"] ?>" sort-status="<?= $status ?>"
