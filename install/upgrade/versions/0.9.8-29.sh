...
if [ -f /usr/local/devcp/data/firewall/ports.conf ]; then
	rm -f /usr/local/devcp/data/firewall/ports.conf
fi

# Move clamav to proper location - https://goo.gl/zNuM11
if [ ! -d /usr/local/devcp/web/edit/server/clamav-daemon ]; then
	mv /usr/local/devcp/web/edit/server/clamd /usr/local/web/edit/server/clamav-daemon
fi
