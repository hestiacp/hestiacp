...
#===========================================================================#
#                                                                           #
# DevIT Control Panel - System Health Check and Repair Function Library    #
#                                                                           #
#===========================================================================#

# Read known configuration keys from $DevIT/conf/defaults/$system.conf
function read_kv_config_file() {
	local system=$1

	if [ ! -f "$DevIT/conf/defaults/$system.conf" ]; then
		write_kv_config_file $system
	fi
	while read -r str; do
		echo "$str"
	done < <(cat $DevIT/conf/defaults/$system.conf)
	unset system
}

# Write known configuration keys to $DevIT/conf/defaults/
function write_kv_config_file() {
	# Ensure configuration directory exists
	if [ ! -d "$DevIT/conf/defaults/" ]; then
		mkdir "$DevIT/conf/defaults/"
	fi

	# Remove previous known good configuration
	if [ -f "$DevIT/conf/defaults/$system.conf" ]; then
		rm -f $DevIT/conf/defaults/$system.conf
	fi

	touch $DevIT/conf/defaults/$system.conf
	for key in $known_keys; do
		echo $key >> $DevIT/conf/defaults/$system.conf
	done
}
...
# Replaces $DevIT/conf/DevIT.conf with "known good defaults" file ($DevIT/conf/defaults/DevIT.conf)
function syshealth_restore_system_config() {
	if [ -f "$DevIT/conf/defaults/DevIT.conf" ]; then
		mv $DevIT/conf/DevIT.conf $DevIT/conf/DevIT.conf.old
		cp $DevIT/conf/defaults/DevIT.conf $DevIT/conf/DevIT.conf
		rm -f $DevIT/conf/DevIT.conf.old
	else
...
function check_key_exists() {
	grep -e "^$1=" $DevIT/conf/DevIT.conf
}

# Repair System Configuration
# Adds missing variables to $DevIT/conf/DevIT.conf with safe default values
function syshealth_repair_system_config() {
	# Release branch
	if [[ -z $(check_key_exists 'RELEASE_BRANCH') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: RELEASE_BRANCH ('release')"
		$BIN/v-change-sys-config-value 'RELEASE_BRANCH' 'release'
	fi
	# Webmail alias
	if [ -n "$IMAP_SYSTEM" ]; then
		if [[ -z $(check_key_exists 'WEBMAIL_ALIAS') ]]; then
			echo "[ ! ] Adding missing variable to DevIT.conf: WEBMAIL_ALIAS ('webmail')"
	...
	if [ -n "$DB_SYSTEM" ]; then
		if [ "$DB_SYSTEM" = "mysql" ]; then
			if [[ -z $(check_key_exists 'DB_PMA_ALIAS') ]]; then
				echo "[ ! ] Adding missing variable to DevIT.conf: DB_PMA_ALIAS ('phpmyadmin)"
				$BIN/v-change-sys-config-value 'DB_PMA_ALIAS' 'phpmyadmin'
			fi
		fi
		if [ "$DB_SYSTEM" = "pgsql" ]; then
			if [[ -z $(check_key_exists 'DB_PGA_ALIAS') ]]; then
				echo "[ ! ] Adding missing variable to DevIT.conf: DB_PGA_ALIAS ('phppgadmin')"
	...
	if [[ -z $(check_key_exists 'BACKUP_GZIP') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: BACKUP_GZIP ('4')"
		$BIN/v-change-sys-config-value 'BACKUP_GZIP' '4'
	fi

	# Theme
	if [[ -z $(check_key_exists 'THEME') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: THEME ('dark')"
		$BIN/v-change-sys-config-value 'THEME' 'dark'
	fi

	# Default language
	if [[ -z $(check_key_exists 'LANGUAGE') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: LANGUAGE ('en')"
		$BIN/v-change-sys-language 'LANGUAGE' 'en'
	fi

	# Disk Quota
	if [[ -z $(check_key_exists 'DISK_QUOTA') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: DISK_QUOTA ('no')"
		$BIN/v-change-sys-config-value 'DISK_QUOTA' 'no'
	fi

	# CRON daemon
	if [[ -z $(check_key_exists 'CRON_SYSTEM') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: CRON_SYSTEM ('cron')"
		$BIN/v-change-sys-config-value 'CRON_SYSTEM' 'cron'
	fi

	# Backend port
	if [[ -z $(check_key_exists 'BACKEND_PORT') ]]; then
		ORIGINAL_PORT=$(sed -ne "/listen/{s/.*listen[^0-9]*\([0-9][0-9]*\)[ \t]*ssl\;/\1/p;q}" "$DevIT/nginx/conf/nginx.conf")
		echo "[ ! ] Adding missing variable to DevIT.conf: BACKEND_PORT ('$ORIGINAL_PORT')"
	...
	if [[ -z $(check_key_exists 'LOGIN_STYLE') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: LOGIN_STYLE ('default')"
	...
	if [[ -z $(check_key_exists 'WEBMAIL_SYSTEM') ]]; then
		if [ -d "/var/lib/roundcube" ]; then
			echo "[ ! ] Adding missing variable to DevIT.conf: WEBMAIL_SYSTEM ('roundcube')"
			$BIN/v-change-sys-config-value "WEBMAIL_SYSTEM" "roundcube"
		else
			echo "[ ! ] Adding missing variable to DevIT.conf: WEBMAIL_SYSTEM ('')"
	...
	if [[ -z $(check_key_exists 'INACTIVE_SESSION_TIMEOUT') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: INACTIVE_SESSION_TIMEOUT ('60')"
	...
	if [[ -z $(check_key_exists 'ENFORCE_SUBDOMAIN_OWNERSHIP') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: ENFORCE_SUBDOMAIN_OWNERSHIP ('yes')"
		$BIN/v-change-sys-config-value "ENFORCE_SUBDOMAIN_OWNERSHIP" "yes"
	fi

	if [[ -z $(check_key_exists 'API') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: API ('no')"
		$BIN/v-change-sys-config-value "API" "no"
	fi

	# Enable API V2
	if [[ -z $(check_key_exists 'API_SYSTEM') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: API_SYSTEM ('0')"
		$BIN/v-change-sys-config-value "API_SYSTEM" "0"
	fi

	# API access allowed IP's
	if [ "$API" = "yes" ]; then
		check_api_key=$(grep "API_ALLOWED_IP" $DevIT/conf/DevIT.conf)
		if [ -z "$check_api_key" ]; then
			if [[ -z $(check_key_exists 'API_ALLOWED_IP') ]]; then
				echo "[ ! ] Adding missing variable to DevIT.conf: API_ALLOWED_IP ('allow-all')"
	...
	if [[ -z $(check_key_exists 'POLICY_USER_DELETE_LOGS') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: POLICY_USER_DELETE_LOGS ('yes')"
	...
	if [[ -z $(check_key_exists 'POLICY_USER_EDIT_DETAILS') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: POLICY_USER_EDIT_DETAILS ('yes')"
	...
	if [[ -z $(check_key_exists 'POLICY_USER_EDIT_DNS_TEMPLATES') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: POLICY_USER_EDIT_DNS_TEMPLATES ('yes')"
	...
	if [[ -z $(check_key_exists 'POLICY_USER_EDIT_WEB_TEMPLATES') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: POLICY_USER_EDIT_WEB_TEMPLATES ('yes')"
		$BIN/v-change-sys-config-value "POLICY_USER_EDIT_WEB_TEMPLATES" "yes"
	fi
	# View user logs
	if [[ -z $(check_key_exists 'POLICY_USER_VIEW_LOGS') ]]; then
		echo "[ ! ] Adding missing variable to DevIT.conf: POLICY_USER_VIEW_LOGS ('yes')"
	...
	while IFS='= ' read -r lhs rhs; do
		...
		check_ckey=$(grep "^$lhs='" "$DevIT/conf/DevIT.conf.new")
		if [ -z "$check_ckey" ]; then
			echo "$lhs='$rhs'" >> "$DevIT/conf/DevIT.conf.new"
		else
			sed -i "s|^$lhs=.*|$lhs='$rhs'|g" "$DevIT/conf/DevIT.conf.new"
		fi
	done < $DevIT/conf/DevIT.conf

	cmp --silent $DevIT/conf/DevIT.conf $DevIT/conf/DevIT.conf.new
	if [ $? -ne 0 ]; then
		echo "[ ! ] Duplicated keys found repair config"
		rm $DevIT/conf/DevIT.conf
		cp $DevIT/conf/DevIT.conf.new $DevIT/conf/DevIT.conf
		rm $DevIT/conf/DevIT.conf.new
	fi
...
# Add default cron jobs to "DevITweb" user account's cron tab
function syshealth_repair_system_cronjobs() {
	min=$(gen_pass '012345' '2')
	hour=$(gen_pass '1234567' '1')
	echo "MAILTO=$email" > /var/spool/cron/crontabs/DevITweb
...
# Adapt Port Listing in DevIT NGINX Backend
# Activates or deactivates port listing on IPV4 or/and IPV6 network interfaces
function syshealth_adapt_DevIT_nginx_listen_ports() {
	...
	NGINX_CONF="/usr/local/DevIT/nginx/conf/nginx.conf"
	if [ -z "$ipv4_scope_global" ]; then
