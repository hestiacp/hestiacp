...
# Vesta Amazon AMI installer v.01
...
software_main="nginx httpd mod_ssl mod_fcgid which php php-common
php-cli php-bcmath php-gd php-imap php-mbstring php-mcrypt php-fpm
php-mysql php-pdo php-soap php-tidy php-xml php-xmlrpc  php-pgsql
vsftpd bind bind-utils bind-libs exim dovecot expect bc jwhois
clamd spamassassin mysql mysql-server postgresql vim-common
postgresql-server postgresql-contrib e2fsprogs openssh-clients
ImageMagick curl mc screen ftp zip unzip flex sqlite pcre sudo
mailx lsof tar telnet rrdtool net-tools ntp GeoIP freetype fail2ban"
software_aux="mod_ruid2 mod_extract_forwarded awstats proftpd
roundcubemail phpMyAdmin phpPgAdmin vesta vesta-nginx vesta-php"
...
if [ -z "$vpass" ]; then
    vpass=$(curl http://169.254.169.254/latest/meta-data/instance-id)
fi
...
# Excluding packages
if [ "$nginx" = 'no'  ]; then
    software_main=$(echo "$software_main" | sed -e "s/^nginx//")
fi
if [ "$apache" = 'no' ]; then
    software_main=$(echo "$software_main" | sed -e "s/httpd//")
    software_main=$(echo "$software_main" | sed -e "s/mod_ssl//")
    software_main=$(echo "$software_main" | sed -e "s/mod_fcgid//")
    software_aux=$(echo  "$software_aux"  | sed -e "s/mod_ruid2//")
fi
if [ "$phpfpm" = 'no' ]; then
    software_main=$(echo "$software_main" | sed -e "s/php-fpm//")
fi
if [ "$vsftpd" = 'no' ]; then
    software_main=$(echo "$software_main" | sed -e "s/vsftpd//")
fi
if [ "$proftpd" = 'no' ]; then
    software_aux=$(echo  "$software_aux"  | sed -e "s/proftpd//")
fi
if [ "$named" = 'no' ]; then
    software_main=$(echo "$software_main" | sed -e "s/bind //")
fi
if [ "$exim" = 'no' ]; then
    software_main=$(echo "$software_main" | sed -e "s/exim//")
    software_main=$(echo "$software_main" | sed -e "s/dovecot//")
    software_main=$(echo "$software_main" | sed -e "s/clamd//")
    software_main=$(echo "$software_main" | sed -e "s/clamav-server//")
    software_main=$(echo "$software_main" | sed -e "s/clamav-update//")
    software_main=$(echo "$software_main" | sed -e "s/spamassassin//")
    software_main=$(echo "$software_main" | sed -e "s/dovecot//")
    software_aux=$(echo  "$software_aux"  | sed -e "s/roundcubemail//")
fi
if [ "$clamd" = 'no' ]; then
    software_main=$(echo "$software_main" | sed -e "s/clamd//")
    software_main=$(echo "$software_main" | sed -e "s/clamav-server//")
...
if [ "$mysql" = 'no' ]; then
    software_main=$(echo "$software_main" | sed -e 's/mysql //')
    software_main=$(echo "$software_main" | sed -e 's/mysql-server//')
    software_main=$(echo "$software_main" | sed -e 's/mariadb //')
    software_main=$(echo "$software_main" | sed -e 's/mariadb-server//')
    software_main=$(echo "$software_main" | sed -e 's/php-mysql//')
    software_aux=$(echo  "$software_aux"  | sed -e 's/phpMyAdmin//')
    software_aux=$(echo  "$software_aux"  | sed -e 's/roundcubemail//')
fi
if [ "$postgresql" = 'no' ]; then
    software_main=$(echo "$software_main" | sed -e 's/postgresql //')
    software_main=$(echo "$software_main" | sed -e 's/postgresql-server//')
    software_main=$(echo "$software_main" | sed -e 's/postgresql-contrib//')
    software_main=$(echo "$software_main" | sed -e 's/php-pgsql//')
    software_aux=$(echo  "$software_aux"  | sed -e 's/phpPgAdmin//')
fi
if [ "$iptables" = 'no' ] || [ "$fail2ban" = 'no' ]; then
    software_main=$(echo "$software_main" | sed -e 's/fail2ban//')
fi
...
yum -y --disableplugin=priorities --disablerepo=* --enablerepo="*main,*updates,nginx" \
	install $software_main
check_result $? "yum install failed"
yum -y --disableplugin=priorities --disablerepo=* --enablerepo="*main,*updates,nginx,epel,vesta" \
	install $software_aux
check_result $? "yum install failed"
