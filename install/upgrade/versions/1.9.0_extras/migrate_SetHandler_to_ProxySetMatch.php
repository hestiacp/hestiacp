...
function differ(string $old, string $new)
{
    ...
    $diff = shell_exec("git diff --no-index --color=always " . escapeshellarg($oldPath) . " " . escapeshellarg($newPath));
    fclose($oldFile);
    fclose($newFile);
    return $diff;
}
...
$conf_files = array_filter($conf_files, function (string $file): bool {
    ...
    if (!is_file($file)) {
        // should never happen.
        throw new \LogicException("Glob say this file exist, but php is_file does not, WTF? " . var_export($file, true));
    }
    return true;
});
...
SAMPLE;
    $ret = preg_match_all('/(?:$|\n)(?<PhpHandlerSection>\s*\\<FilesMatch\s+\\\\\\.php\\$\\>[\s\S]+?\\<\\/FilesMatch\\>)/', $original_conf_string, $matches, PREG_OFFSET_CAPTURE);
    ...
    $documentRootMatchRet = preg_match('/^\s*DocumentRoot\s+([^\s]*)/m', $original_conf_string, $documentRootMatch);
    if ($documentRootMatchRet === false || $documentRootMatchRet === 0 || empty($documentRootMatch[1])) {
    ...
    foreach ($php_handler_sections as $php_handler_section) {
        $php_handler_section_sample = array(
            0 => '
            <FilesMatch \\.php$>
                SetHandler "proxy:unix:/run/php/php8.2-fpm-idn-tést.eu.sock|fcgi://localhost"
            </FilesMatch>',
        ...
        if (empty($proxyMatch[1])) {
            throw new \LogicException("Failed to extract proxy:unix:... from php handler section in '$conf_file' - investigate");
        }
        $proxy = $proxyMatch[1]; // string(59) "unix:/run/php/php8.2-fpm-idn-tést.eu.sock|fcgi://localhost"
        $proxy = strtr($proxy, array(
        ...
        $proxyString = "\n{$leading_spaces}" . 'ProxyPassMatch ^/(.*\\.php)(?:$|\\?) "' . $proxy . '" nocanon' . "\n";
        $modified_conf_string = substr_replace($modified_conf_string, $proxyString, $php_handler_section[1], strlen($php_handler_section[0]));
    }
