...
<div class="toolbar">
	<div class="toolbar-inner">
		<div class="toolbar-buttons">
			...
			?>
			<a class="button button-secondary" id="btn-back" href="/list/backup/incremental/?snapshot=<?= tohtml( htmlentities($_GET["snapshot"])) ?>&browse=yes&folder=<?=htmlentities($files[0]['path'])?>/../&token=<?= tohtml($_SESSION["token"]) ?>"><i class="fas fa-arrow-left icon-blue"></i><?= tohtml( _("Back")) ?></a>
			<?php }else{
			?>
			...
			if($file['path'] != '/home/'.$user_plain){
				if($file['path'] != '/home/'.$user_plain && $file['path'] == $files[0]['path']){
					...
					<div class="units-table-row js-unit">
						...
						<div class="units-table-cell units-table-heading-cell u-text-bold">
								<b><a href="/list/backup/incremental/?snapshot=<?= tohtml( htmlentities($_GET["snapshot"])) ?>&browse=yes&folder=<?=htmlentities($files[0]['path'])?>/../&token=<?= tohtml($_SESSION["token"]) ?>"><i class="fas fa-folder icon-dim u-mr5"></i>..</a></b>
						</div>
						<div class="units-table-cell">
						</div>
						<div class="units-table-cell">
				...
				?>
					<div class="units-table-row js-unit">
						...
						<div class="units-table-cell">
							<div>
								...
								<?php
									...
									?>
										<b><a href="/list/backup/incremental/?snapshot=<?= tohtml( htmlentities($_GET["snapshot"])) ?>&browse=yes&folder=<?=htmlentities($file['path'])?>&token=<?= tohtml($_SESSION["token"]) ?>"><i class="fas fa-folder icon-dim u-mr5"></i><?= tohtml($file['name']) ?></a></b>
									<?php
									}
								}else{
									?>
						...
						<div class="units-table-cell">
							<a href="/schedule/restore/incremental/?snapshot=<?= tohtml( htmlentities($_GET["snapshot"])) ?>&type=file&object=<?=htmlentities($file['path'])?>&token=<?= tohtml($_SESSION["token"]) ?>" title="<?= tohtml( _("Restore")) ?>">
