...
function syshealth_update_user_config_format() {
	...
	# Create array of known keys in configuration file
	system="user"
	known_keys="NAME PACKAGE CONTACT CRON_REPORTS MD5 RKEY TWOFA QRCODE PHPCLI ROLE SUSPENDED SUSPENDED_USERS SUSPENDED_WEB SUSPENDED_DNS SUSPENDED_MAIL SUSPENDED_DB SUSPENDED_CRON IP_AVAIL IP_OWNED U_USERS U_DISK U_DISK_DIRS U_DISK_WEB U_DISK_MAIL U_DISK_DB U_BANDWIDTH U_WEB_DOMAINS U_WEB_SSL U_WEB_ALIASES U_DNS_DOMAINS U_DNS_RECORDS U_MAIL_DKIM U_MAIL_DKIM U_MAIL_ACCOUNTS U_MAIL_DOMAINS U_MAIL_SSL U_DATABASES U_CRON_JOBS U_BACKUPS LANGUAGE THEME NOTIFICATIONS PREF_UI_SORT PREF_SUBDOMAIN_GROUPING TIME DATE"
	write_kv_config_file
	unset system
	unset known_keys
...
function syshealth_repair_system_config() {
	...
	# Subdomain grouping in the Web domain list
	if [[ -z $(check_key_exists 'SUBDOMAIN_GROUPING') ]]; then
		echo "[ ! ] Adding missing variable to hestia.conf: SUBDOMAIN_GROUPING ('no')"
		$BIN/v-change-sys-config-value "SUBDOMAIN_GROUPING" "no"
	fi
