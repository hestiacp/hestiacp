...
is_web_alias_new() {
	grep -wH "$1" $DevIT/data/users/*/web.conf | while read -r line; do
...
is_mail_domain_new() {
	...
	for mail_reserved in $(echo "mail $WEBMAIL_ALIAS"); do
		if [ -n "$(ls $DevIT/data/users/*/mail/$mail_reserved.$1.conf 2> /dev/null)" ]; then
			if [ "$2" == 'mail' ]; then
				check_result "$E_EXISTS" "Required subdomain \"$mail_reserved.$1\" already exists"
			fi
		fi
		if [ -n "$(ls $DevIT/data/users/*/mail/$mail_nosub.conf 2> /dev/null)" ] && [ "$mail_sub" = "$mail_reserved" ]; then
			if [ "$2" == 'mail' ]; then
...
add_mail_ssl_config() {
	...
	if [ ! -d "$DevIT/ssl/mail" ]; then
		mkdir -p $DevIT/ssl/mail
	fi
	...
	# Add certificate to DevIT user configuration data directory
	...
	if [ -n "$mail_cert_match" ]; then
		...
		ln -s $HOMEDIR/$user/conf/mail/$domain/ssl/$domain.pem $DevIT/ssl/mail/$domain.crt
		ln -s $HOMEDIR/$user/conf/mail/$domain/ssl/$domain.key $DevIT/ssl/mail/$domain.key
	fi
	...
	# Add domain SSL configuration to exim4
	ln -s $HOMEDIR/$user/conf/mail/$domain/ssl/$domain.pem $DevIT/ssl/mail/mail.$domain.crt
	ln -s $HOMEDIR/$user/conf/mail/$domain/ssl/$domain.key $DevIT/ssl/mail/mail.$domain.key
	...
	chmod -R 0644 $DevIT/ssl/mail/*
	chown -h $user:mail $DevIT/ssl/mail/*
}
...
del_mail_ssl_config() {
	...
	rm -f $HOMEDIR/$user/conf/mail/$domain/ssl/*
	if [ -n "$mail_cert_match" ]; then
		rm -f $DevIT/ssl/mail/$domain.crt $DevIT/ssl/mail/$domain.key
	fi
	rm -f $DevIT/ssl/mail/mail.$domain.crt $DevIT/ssl/mail/mail.$domain.key
}
...
is_valid_extension() {
	if [ ! -e "$DevIT/data/extensions/public_suffix_list.dat" ]; then
		mkdir $DevIT/data/extensions/
		chmod 750 $DevIT/data/extensions/
		/usr/bin/wget --tries=3 --timeout=15 --read-timeout=15 --waitretry=3 --no-dns-cache --quiet -O $DevIT/data/extensions/public_suffix_list.dat https://raw.githubusercontent.com/publicsuffix/list/master/public_suffix_list.dat
	fi
	test_domain=$(idn2 -d "$1")
	extension=$(/bin/echo "${test_domain}" | /usr/bin/rev | /usr/bin/cut -d "." --output-delimiter="." -f 1 | /usr/bin/rev)
	exten=$(grep "^$extension\$" $DevIT/data/extensions/public_suffix_list.dat)
}
...
is_base_domain_owner() {
	for object in ${1//,/ }; do
		if [ "$object" != "none" ]; then
			...
			web=$(grep -F -H -h "DOMAIN='$basedomain'" $DevIT/data/users/*/web.conf)
