...
if [ -f /etc/nginx/nginx.conf ]; then
	...
	sed -i -E 's/(.*keepalive_requests\s{1,})10000;/\11000;/' /etc/nginx/nginx.conf /usr/local/DevIT/nginx/conf/nginx.conf
fi

# Fix security issue wit FPM pools
if [ -z "$(grep ^DevITmail: /etc/passwd)" ]; then
	echo "[ * ] Limit permissions www.conf and dummy.conf"
	/usr/sbin/useradd "DevITmail" -c "$email" --no-create-home

	sed -i "s/user = www-data/user = DevITmail/g" /etc/php/*/fpm/pool.d/www.conf

	php_versions=$($BIN/v-list-sys-php plain)
	# Substitute php-fpm service name formats
	for version in $php_versions; do
		cp -f $DevIT_INSTALL_DIR/php-fpm/dummy.conf /etc/php/$version/fpm/pool.d/
