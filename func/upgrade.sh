...
upgrade_send_notification_to_email () {
    if [ "$UPGRADE_SEND_EMAIL" = "true" ]; then
        ...
        admin_email=$($HESTIA/bin/v-list-user "$ROOT_USER" json | grep "CONTACT" | cut -d'"' -f4)
