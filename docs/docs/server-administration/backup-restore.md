...
Currently DevITCP only support restoring backups made using:
...
For safety reasons, DevIT takes into account 2x the user’s disk usage when creating a backup. Therefore, before starting a backup, we check how much disk usage a user has left. If you encounter this error, you can do one of the following to solve the issue:
...
Once the config has been saved you can setup DevIT with the following command:
...
```bash
v-add-backup-host 'rclone' 'b2' '' '' 'DevITcp'
```
...
Since 1.9 we include Rclone by default in DevIT Installation. Run `rclone config` as the `root` user and then select the option `n`. Follow the instruction on the screen, then save when completed.
...
:::warning
A new restic repository is initiated on the first time you run this command. An encryption key is generated at the same time in /usr/local/DevIT/data/users/{users}/restic.conf. Please make sure to backup this file somewhere incase the server gets comprimised or the user gets deleted. Without this "secret" key we don't provide any method to restore the user data. This is the reason why we alway advice to keep the orignal backup still working.
:::
