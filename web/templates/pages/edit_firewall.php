...
<div class="container">
    ...
    <form id="main-form" name="v_edit_firewall" method="post" class="<?= $v_status ?>">
        ...
        <?php
        $ipset_lists_attr = htmlspecialchars($ipset_lists_json, ENT_QUOTES, "UTF-8");
        $port_placeholder = _("All ports: 0, Range: 80-82, List: 80,443,8080,8443");
        $v_port_clean = htmlentities(trim($v_port, "'"));
        $v_ip_clean = htmlentities(trim($v_ip, "'"));
        $v_comment_clean = htmlentities(trim($v_comment, "'"));
        ?>
        ...
        <div class="form-container">
            ...
            <div class="u-mb10">
                ...
                <select class="form-select" name="v_protocol" id="v_protocol">
                    <option value="TCP" <?php if ((!empty($v_protocol)) && ($v_protocol == "TCP")) {
                                            echo 'selected';
                                        } ?>>TCP</option>
                    <option value="UDP" <?php if ((!empty($v_protocol)) && ($v_protocol == "UDP")) {
                                            echo 'selected';
                                        } ?>>UDP</option>
                    <option value="ICMP" <?php if ((!empty($v_protocol)) && ($v_protocol == "ICMP")) {
                                                echo 'selected';
                                         } ?>>ICMP</option>
                </select>
            </div>
