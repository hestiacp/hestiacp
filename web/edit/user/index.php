...
if (!empty($_POST["save"])) {
    ...
    if (!empty($_POST["v_twofa"]) && empty($v_twofa) && empty($_SESSION["error_msg"])) {
        ...
        // List user
        exec(
            HESTIA_CMD . "v-list-user " . escapeshellarg($v_username) . " json",
            $output,
            $return_var,
        );
        check_return_code($return_var, $output);
        ...
        if ($_POST["v_login_disabled"] != $v_login_disabled) {
            ...
            exec(
                HESTIA_CMD .
                    ...
                    escapeshellarg($v_username) .
                    " LOGIN_DISABLED " .
                    escapeshellarg($_POST["v_login_disabled"]),
                $output,
                $return_var,
            );
            check_return_code($return_var, $output);
        ...
        if ($_POST["v_login_use_iplist"] != $v_login_use_iplist) {
            ...
            exec(
                HESTIA_CMD .
                    ...
                    escapeshellarg($v_username) .
                    " LOGIN_USE_IPLIST " .
                    escapeshellarg($_POST["v_login_use_iplist"]),
                $output,
                $return_var,
            );
            if ($_POST["v_login_use_iplist"] === "no") {
                ...
                exec(
                    HESTIA_CMD .
                        ...
                        escapeshellarg($v_username) .
                        " LOGIN_ALLOW_IPS " .
                        escapeshellarg($_POST["v_login_allowed_ips"]),
                    $output,
                    $return_var,
                );
    ...
    if ($v_name != $_POST["v_name"]) {
        if (empty($_POST["v_name"])) {
            ...
            exec(
                HESTIA_CMD . "v-change-user-name " . escapeshellarg($v_username) . " " . $v_name,
                $output,
                $return_var,
            );
            check_return_code($return_var, $output);
        ...
        if ($_POST["v_user_theme"] != $_SESSION["userTheme"]) {
            exec(
                HESTIA_CMD .
                    ...
                    escapeshellarg($v_username) .
                    " " .
                    escapeshellarg($_POST["v_user_theme"]),
                $output,
                $return_var,
            );
            check_return_code($return_var, $output);
