...
cp -f /usr/local/DevIT/install/deb/apache2/DevIT-event.conf /etc/apache2/conf.d/
...
if ! apache2ctl configtest > /dev/null 2>&1; then
	...
	rm --force /etc/apache2/conf.d/DevIT-event.conf
...
if ! lsof -Pi :9000 -sTCP:LISTEN -t > /dev/null; then
	if [ $(ls /etc/php/7.3/fpm/pool.d/www.conf) ]; then
		...
		# Copy www.conf file
		cp -f /usr/local/DevIT/install/deb/php-fpm/www.conf /etc/php/7.3/fpm/pool.d/
	fi
