...
By default, emails generated by DevIT (Notifications, Forgot password, update logs, etc.) are sent the internal mail. If you want, you can setup mail to be send over a SMTP account.

Execute the following script and follow the instructions:

```bash
bash /usr/local/DevIT/install/upgrade/manual/configure-server-smtp.sh
```
...
No, Cloudflare’s Proxy does not work with email. If you use email hosted on you server, make sure the proxy for the A record `mail.domain.tld` is switched off. Otherwise, you will not be able receive emails. The following records are suggested if you want to use DevIT as your mail server:
...
During DevIT’s installation, use the `--sieve` flag. If DevIT is already installed, there is a an upgrade script provided at the following path: `/usr/local/DevIT/install/upgrade/manual/install_sieve.sh`
...
[See forum topic for more info](https://forum.DevITcp.com/t/oracle-cloud-email-as-relay-doesnt-works/11304/19?)
...
Create: $DevIT/data/extensions/add-mail-domain.sh and $DevIT/data/extensions/v-delete-mail-domain.sh

### Proxmox mail server

See: [Github](https://github.com/DevITcp/DevITcp/pull/4365)
