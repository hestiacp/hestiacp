...
# DevIT Control Panel upgrade script for target version 1.7.3
...
if grep -q 'drop    message       = Helo name contains an IP address (HELO was $sender_helo_name) and not is valid' /etc/exim4/exim4.conf.template; then
	...
	patch /etc/exim4/exim4.conf.template $DevIT/install/upgrade/patch/3462-exim-helo-autenticted-users.patch
fi
