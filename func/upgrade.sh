...
#===========================================================================#
#                                                                           #
# DevIT Control Panel - Upgrade Function Library                           #
#                                                                           #
#===========================================================================#

# Import system health check and repair library
# shellcheck source=/usr/local/DevIT/func/syshealth.sh
...
add_upgrade_message() {
	if [ -f "$DevIT_BACKUP/message.log" ]; then
		echo -e $1 >> $DevIT_BACKUP/message.log
		echo -e "\n\n" >> $DevIT_BACKUP/message.log
	else
		echo -e $1 > $DevIT_BACKUP/message.log
	fi
}
...
upgrade_health_check() {
	...
	# Perform basic health check against DevIT.conf to ensure that
	...
	if [ -z "$VERSION" ]; then
		...
		echo
		echo "[ ! ] Unable to detect installed version of DevIT Control Panel."
...
upgrade_welcome_message() {
	...
	echo "                                                                              "
	echo "                    DevIT Control Panel Software Update                      "
	...
	echo ""
	echo "Backup directory: $DevIT_BACKUP/"
	echo "Installation log: $LOG"
}

upgrade_welcome_message_log() {
	echo "============================================================================="
	echo "DevIT Control Panel Software Update Log"
	echo "============================================================================="
	echo
...
upgrade_complete_message() {
	...
	echo "https://github.com/DevITcp/DevITcp/issues                                  "
	echo
	echo "Read the release notes to learn about new fixes and features:                "
	echo "https://github.com/DevITcp/DevITcp/blob/release/CHANGELOG.md               "
	echo
	echo "We hope that you enjoy using this version of DevIT Control Panel,           "
	echo "have a wonderful day!                                                        "
	echo
	echo "Sincerely,                                                                   "
	echo "The DevIT Control Panel development team                                    "
	echo
	echo "Web:      https://www.DevITcp.com/                                          "
	echo "Docs:     https://docs.DevITcp.com/										   "
	echo "Forum:    https://forum.DevITcp.com/                                        "
	echo "GitHub:   https://github.com/DevITcp/DevITcp/                              "
	echo
	echo "Help support the DevIT Control Panel project by donating via PayPal:        "
	echo "https://www.DevITcp.com/donate                                              "
	echo
...
upgrade_get_version() {
	# Retrieve new version number for DevIT Control Panel from .deb package
	new_version=$(dpkg -l | awk '$2=="DevIT" { print $3 }')
}

upgrade_set_version() {
	# Set new version number in DevIT.conf
	$BIN/v-change-sys-config-value "VERSION" "$@"
}

upgrade_set_branch() {
	# Set branch in DevIT.conf
...
upgrade_send_notification_to_panel() {
	...
	if [[ "$new_version" =~ "alpha" ]]; then
		...
		$BIN/v-add-user-notification "$ROOT_USER" 'Development snapshot installed' '<p><span class="u-text-bold">Version:</span> '$new_version'<br><span class="u-text-bold">Code Branch:</span> '$RELEASE_BRANCH'</p><p>Please report any bugs by <a href="https://github.com/DevITcp/DevITcp/issues" target="_blank">opening an issue on GitHub</a>, and feel free to share your feedback on our <a href="https://forum.DevITcp.com" target="_blank">discussion forum</a>.</p><p><i class="fas fa-heart icon-red"></i> The DevIT Control Panel development team</p>'
	elif [[ "$new_version" =~ "beta" ]]; then
		# Send feedback notification for beta releases
		$BIN/v-add-user-notification "$ROOT_USER" 'Thank you for testing DevIT Control Panel '$new_version'.' '<p>Please share your feedback with our development team through our <a href="https://forum.DevITcp.com" target="_blank">discussion forum</a>.</p><p>Found a bug? <a href="https://github.com/DevITcp/DevITcp/issues" target="_blank">Open an issue on GitHub</a>!</p><p><i class="fas fa-heart icon-red"></i> The DevIT Control Panel development team</p>'
	else
		# Send normal upgrade complete notification for stable releases
		$BIN/v-add-user-notification "$ROOT_USER" 'Upgrade complete' '<p>DevIT Control Panel has been updated to <span class="u-text-bold">v'$new_version'</span>.</p><p><a href="https://github.com/DevITcp/DevITcp/blob/release/CHANGELOG.md" target="_blank">View release notes</a></p><p>Please report any bugs by <a href="https://github.com/DevITcp/DevITcp/issues" target="_blank">opening an issue on GitHub</a>.</p><p class="u-text-bold">Have a wonderful day!</p><p><i class="fas fa-heart icon-red"></i> The DevIT Control Panel development team</p>'
	fi
}
...
upgrade_send_notification_to_email() {
	...
	if [ "$UPGRADE_SEND_EMAIL" = "true" ]; then
		...
		message_tmp_file="/tmp/DevIT-upgrade-complete.txt"

		# Create temporary file
		touch $message_tmp_file

		# Write message to file
		echo "$HOSTNAME has been upgraded from DevIT Control Panel v$VERSION to v${new_version}." >> $message_tmp_file
		...
		if [[ -f "$DevIT_BACKUP/message.log" ]]; then
			...
			echo "" >> $message_tmp_file
			cat $DevIT_BACKUP/message.log >> $message_tmp_file
			echo "" >> $message_tmp_file
			echo "===================================================" >> $message_tmp_file
			echo "" >> $message_tmp_file
		fi

		echo "What's new: https://github.com/DevITcp/DevITcp/blob/$RELEASE_BRANCH/CHANGELOG.md" >> $message_tmp_file
		echo >> $message_tmp_file
		echo "What to do if you run into issues:" >> $message_tmp_file
		echo "- Check our forums for possible solutions: https://forum.DevITcp.com" >> $message_tmp_file
		echo "- File an issue report on GitHub: https://github.com/DevITcp/DevITcp/issues" >> $message_tmp_file
		echo "" >> $message_tmp_file
		echo "Help support the DevIT Control Panel project by donating via PayPal: https://www.DevITcp.com/donate" >> $message_tmp_file
		echo "===================================================" >> $message_tmp_file
		echo "Have a wonderful day," >> $message_tmp_file
		echo "The DevIT Control Panel development team" >> $message_tmp_file
...
upgrade_config_set_value() {
	if [ -f "$DevIT_BACKUP/upgrade.conf" ]; then
		if [ "$2" = "true" ]; then
			sed -i "s/$1='false'/$1='true'/g" $DevIT_BACKUP/upgrade.conf
		fi
	fi
}

prepare_upgrade_config() {
	mkdir -p $DevIT_BACKUP
	touch $DevIT_BACKUP/upgrade.conf
	while IFS='= ' read -r lhs rhs; do
		if [[ ! $lhs =~ ^\ *# && -n $lhs ]]; then
			...
			echo "$lhs='$rhs'" >> $DevIT_BACKUP/upgrade.conf
		fi
	done < "$DevIT/install/upgrade/upgrade.conf"
}

upgrade_init_backup() {
	# Ensure that backup directories are created
	# DevIT Control Panel configuration files
	mkdir -p $DevIT_BACKUP/conf/DevIT/

	# OpenSSL configuration files
	mkdir -p $DevIT_BACKUP/conf/openssl/

	# Hosting Packages
	mkdir -p $DevIT_BACKUP/packages/

	# Domain template files
	mkdir -p $DevIT_BACKUP/templates/

	# System services (apache2, nginx, bind9, vsftpd, etc).
	if [ -n "$WEB_SYSTEM" ]; then
		mkdir -p $DevIT_BACKUP/conf/$WEB_SYSTEM/
	fi
	if [ -n "$IMAP_SYSTEM" ]; then
		mkdir -p $DevIT_BACKUP/conf/$IMAP_SYSTEM/
	fi
	if [ -n "$MAIL_SYSTEM" ]; then
		mkdir -p $DevIT_BACKUP/conf/$MAIL_SYSTEM/
	fi
	if [ -n "$DNS_SYSTEM" ]; then
		mkdir -p $DevIT_BACKUP/conf/$DNS_SYSTEM/
	fi
	if [ -n "$PROXY_SYSTEM" ]; then
		mkdir -p $DevIT_BACKUP/conf/$PROXY_SYSTEM/
	fi
	if [ -n "$DB_SYSTEM" ]; then
		if [[ "$DB_SYSTEM" =~ "mysql" ]]; then
			mkdir -p $DevIT_BACKUP/conf/mysql/
		fi
		if [[ "$DB_SYSTEM" =~ "pgsql" ]]; then
			mkdir -p $DevIT_BACKUP/conf/pgsql/
		fi
	fi
	if [ -n "$FTP_SYSTEM" ]; then
		mkdir -p $DevIT_BACKUP/conf/$FTP_SYSTEM/
	fi
	if [ -n "$FIREWALL_SYSTEM" ]; then
		mkdir -p $DevIT_BACKUP/conf/$FIREWALL_SYSTEM/
	fi
	if [ -n "$FIREWALL_EXTENSION" ]; then
		mkdir -p $DevIT_BACKUP/conf/$FIREWALL_EXTENSION/
	fi
	if [ -e "/etc/ssh/sshd_config" ]; then
		mkdir -p $DevIT_BACKUP/conf/ssh/
	fi
	if [ -d "/etc/roundcube/" ]; then
		mkdir -p $DevIT_BACKUP/conf/roundcube/
	fi
	if [ -d "/etc/snappymail/" ]; then
		mkdir -p $DevIT_BACKUP/conf/snappymail/
	fi
	if [ -d "/etc/phpmyadmin/" ]; then
		mkdir -p $DevIT_BACKUP/conf/phpmyadmin/
	fi
	if [ -d "/etc/phppgadmin/" ]; then
		mkdir -p $DevIT_BACKUP/conf/phppgadmin/
	fi
}

upgrade_init_logging() {
	# Set log file path
	LOG="$DevIT_BACKUP/hst-upgrade-$(date +%d%m%Y%H%M).log"
	...
	if [[ "$new_version" =~ "alpha" ]]; then
		$BIN/v-log-action "system" "Warning" "Updates" "Development build for testing purposes only. Report bugs at https://github.com/DevITcp/DevITcp/issues/."
	fi
	if [[ "$new_version" =~ "beta" ]]; then
		$BIN/v-log-action "system" "Warning" "Updates" "Beta release. Please report bugs at https://github.com/DevITcp/DevITcp/issues/."
	fi
}
...
upgrade_start_backup() {
	...
	cp -fr $DevIT/data/packages/* $DevIT_BACKUP/packages/

	if [ "$DEBUG_MODE" = "true" ]; then
		echo "      - Templates"
	fi
	cp -fr $DevIT/data/templates/* $DevIT_BACKUP/templates/
	...
	cp -fr $DevIT/conf/* $DevIT_BACKUP/conf/DevIT/

	# OpenSSL configuration files
	if [ "$DEBUG_MODE" = "true" ]; then
		echo "      ---- openssl"
	fi
	cp -f /etc/ssl/*.cnf $DevIT_BACKUP/conf/openssl/
	...
	if [ -n "$WEB_SYSTEM" ]; then
		...
		cp -fr /etc/$WEB_SYSTEM/* $DevIT_BACKUP/conf/$WEB_SYSTEM/
	fi
	if [ -n "$PROXY_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- $PROXY_SYSTEM"
		fi
		cp -fr /etc/$PROXY_SYSTEM/* $DevIT_BACKUP/conf/$PROXY_SYSTEM/
	fi
	if [ -n "$IMAP_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- $IMAP_SYSTEM"
		fi
		cp -fr /etc/$IMAP_SYSTEM/* $DevIT_BACKUP/conf/$IMAP_SYSTEM/
	fi
	if [ -n "$MAIL_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- $MAIL_SYSTEM"
		fi
		cp -fr /etc/$MAIL_SYSTEM/* $DevIT_BACKUP/conf/$MAIL_SYSTEM/
	fi
	if [ -n "$DNS_SYSTEM" ]; then
		if [ "$DNS_SYSTEM" = "bind9" ]; then
			...
			cp -fr /etc/bind/* $DevIT_BACKUP/conf/$DNS_SYSTEM/
		fi
	fi
	if [ -n "$DB_SYSTEM" ]; then
		if [[ "$DB_SYSTEM" =~ "mysql" ]]; then
			...
			cp -fr /etc/mysql/* $DevIT_BACKUP/conf/mysql/
		fi
		if [[ "$DB_SYSTEM" =~ "pgsql" ]]; then
			if [ "$DEBUG_MODE" = "true" ]; then
				echo "      ---- pgsql"
			...
			cp -fr /etc/postgresql/* $DevIT_BACKUP/conf/pgsql/
		fi
	fi
	if [ -n "$FTP_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
		...
		if [ "$FTP_SYSTEM" = "vsftpd" ]; then
			cp -f /etc/$FTP_SYSTEM.conf $DevIT_BACKUP/conf/$FTP_SYSTEM/
		fi
		if [ "$FTP_SYSTEM" = "proftpd" ]; then
			cp -f /etc/proftpd/proftpd.conf $DevIT_BACKUP/conf/$FTP_SYSTEM/
		fi
	fi
	if [ -n "$FIREWALL_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
		...
		[ -e "/etc/sysconfig/iptables" ] && cp -f /etc/sysconfig/iptables $DevIT_BACKUP/conf/$FIREWALL_SYSTEM/
		[ -e "/etc/iptables.rules" ] && cp -f /etc/iptables.rules $DevIT_BACKUP/conf/$FIREWALL_SYSTEM/
	fi
	if [ -n "$FIREWALL_EXTENSION" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- $FIREWALL_EXTENSION"
		fi
		cp -f /etc/$FIREWALL_EXTENSION/*.conf $DevIT_BACKUP/conf/$FIREWALL_EXTENSION/
		cp -f /etc/$FIREWALL_EXTENSION/*.local $DevIT_BACKUP/conf/$FIREWALL_EXTENSION/
	fi
	if [ -e "/etc/ssh/sshd_config" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- sshd"
		fi
		cp -fr /etc/ssh/* $DevIT_BACKUP/conf/ssh/
	fi
	if [ -d "/etc/roundcube" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- Roundcube"
		fi
		cp -fr /etc/roundcube/* $DevIT_BACKUP/conf/roundcube
	fi
	if [ -d "/etc/snappymail" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- SnappyMail"
		fi
		cp -fr /etc/snappymail/* $DevIT_BACKUP/conf/snappymail
	fi
	if [ -d "/etc/phpmyadmin" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- phpMyAdmin"
		fi
		cp -fr /etc/phpmyadmin/* $DevIT_BACKUP/conf/phpmyadmin
	fi
	if [ -d "/etc/phppgadmin" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- phppgadmin"
		fi
		cp -fr /etc/phppgadmin/* $DevIT_BACKUP/conf/phppgadmin
	fi
}

upgrade_refresh_config() {
	source_conf "/usr/local/DevIT/conf/DevIT.conf"
}
...
upgrade_start_routine() {
	...
	upgrade_steps=$(ls -v $DevIT/install/upgrade/versions/*.sh)
	for script in $upgrade_steps; do
		declare -a available_versions
		available_versions+=($(echo $script | sed "s|/usr/local/DevIT/install/upgrade/versions/||g" | sed "s|.sh||g"))
	done
	...
	if [ $(check_version $latest_version) -gt $(check_version $VERSION) ]; then
		for version_step in "${available_versions[@]}"; do
			if [ $(check_version $VERSION) -lt $(check_version "$version_step") ]; then
				upgrade_step_message
				source $DevIT/install/upgrade/versions/$version_step.sh
			fi
		done
		...
		echo ""
		echo "[ ! ] The latest version of DevIT Control Panel is already installed."
		echo "      Verifying configuration..."
		echo ""
		if [ -e "$DevIT/install/upgrade/versions/$VERSION.sh" ]; then
			source $DevIT/install/upgrade/versions/$VERSION.sh
		fi
		VERSION="$new_version"
		upgrade_set_version "$VERSION"
		upgrade_refresh_config
	...
	if [ -n "$(echo $DB_SYSTEM | grep -w 'mysql')" ]; then
		...
		if version_ge "$pma_version" "$pma_v"; then
			...
			if [ -e /var/lib/phpmyadmin/blowfish_secret.inc.php ]; then
				chown root:DevITmail /var/lib/phpmyadmin/blowfish_secret.inc.php
				chmod 0640 /var/lib/phpmyadmin/blowfish_secret.inc.php
			fi
			chown DevITmail:DevITmail /usr/share/phpmyadmin/tmp
...
upgrade_filemanager() {
	FILE_MANAGER_CHECK=$(cat $DevIT/conf/DevIT.conf | grep "FILE_MANAGER='false'")
	if [ -z "$FILE_MANAGER_CHECK" ]; then
		if [ -f "$DevIT/web/fm/version" ]; then
			fm_version=$(cat $DevIT/web/fm/version)
		else
			fm_version="1.0.0"
		fi
		if ! version_ge "$fm_version" "$fm_v"; then
			...
			if [ "$UPGRADE_UPDATE_FILEMANAGER_CONFIG" = "true" ]; then
				if [ -e "$DevIT/web/fm/configuration.php" ]; then
					echo "[ ! ] Updating File Manager configuration..."
					# Update configuration.php
					cp -f $DevIT_INSTALL_DIR/filemanager/filegator/configuration.php $DevIT/web/fm/configuration.php

					# Path to the file manager configuration file where the change will be made.
					config_file="$DevIT/web/fm/configuration.php"
...
upgrade_roundcube() {
	if [ -n "$(echo "$WEBMAIL_SYSTEM" | grep -w 'roundcube')" ]; then
		if [ -d "/usr/share/roundcube" ]; then
			...
			echo "      To upgrade to the latest version of Roundcube directly from upstream, from please run the command migrate_roundcube.sh located in: /usr/local/DevIT/install/upgrade/manual/"
		else
...
upgrade_dependencies() {
	echo "[ ! ] Update DevIT PHP dependencies..."
...
upgrade_restart_services() {
	if [ "$UPGRADE_RESTART_SERVICES" = "true" ]; then
		...
		if [ "$WEB_TERMINAL" = "true" ]; then
			...
			$BIN/v-restart-service "DevIT-web-terminal"
		fi
	...
	# Always restart the DevIT Control Panel service
	if [ "$DEBUG_MODE" = "true" ]; then
		echo "      - DevIT"
	fi
	$BIN/v-restart-service DevIT
}
