...
#                                                                           #
# IPv6 dual-stack design (non-invasive):                                    #
#   This file extends the official rebuild logic with IPv6 support while    #
#   preserving 100% backward compatibility with IPv4-only deployments.      #
#                                                                           #
#   Three IP cases are handled transparently in rebuild_web_domain_conf():  #
#     1. IPv4-only  — standard path, no changes to existing behavior        #
#     2. IPv6-only  — local_ip set to [ipv6], templates use it directly     #
#     3. Dual-stack — IPv4 is primary; IPv6 listen lines are injected via   #
#                     sed AFTER config generation (no template changes)     #
#                                                                           #
#   The IPv6 injection block reads IP6 from web.conf (set by               #
#   v-change-web-domain-ipv6) or auto-detects the user's first IPv6 via    #
#   get_user_ipv6(). It never runs on IPv4-only servers (ipv6_addr empty). #
#                                                                           #
#   Mail domain rebuilds (rebuild_mail_domain_conf) read IP6 from           #
#   web.conf and write it to conf/mail/$domain/ipv6 for Exim — same        #
#   non-invasive pattern: new file, no changes to existing mail logic.      #
#                                                                           #
#===========================================================================#
...
rebuild_web_domain_conf() {
	...
	# ---------------------------------------------------------------------------
	# IP resolution — three cases, all handled non-invasively:
	#
	#   Case 1 — IPv4-only or dual-stack domain (IP= is set in web.conf):
	#     The existing is_ip_valid() call resolves local_ip and ip normally.
	#     If IP6 is also set, dual-stack listen lines are injected later (see
	#     the "Dual-stack IPv6 injection" block below). The IPv4 stack is
	#     never touched; only additional listen lines are appended.
	#
	#   Case 2 — IPv6-only domain (IP= empty, IP6= set in web.conf):
	#     local_ip is set to [IP6] (bracket notation required by nginx/Apache).
	#     The address is validated against $HESTIA/data/ips/ before use.
	#     Standard templates receive [ipv6] as %ip%, so no template changes
	#     are needed — the brackets make it valid in VirtualHost directives.
	#
	#   Case 3 — No IP assigned at all (fresh domain or auto-assign):
	#     get_user_ipv6() is tried first to support IPv6-only servers.
	#     Falls back to get_user_ip() for IPv4-only servers.
	#     This mirrors the original auto-assign logic but prefers IPv6 when
	#     available, enabling automatic dual-stack on capable servers.
	# ---------------------------------------------------------------------------
	if [ -n "$IP" ]; then
		is_ip_valid $IP
	elif [ -n "$IP6" ]; then
		...
		# No IP assigned — try IPv6 first (IPv6-only server), then IPv4
		get_user_ipv6
		if [ -n "$ipv6" ]; then
			local_ip="[$ipv6]"
			ip="$ipv6"
	...
	# ---------------------------------------------------------------------------
	# Dual-stack IPv6 injection
	#
	# This block runs AFTER all config files have been generated by the
	# template engine (add_web_config). It surgically inserts IPv6 listen
	# directives into the already-rendered configs using sed, so we never
	# need to modify any existing .tpl / .stpl templates.
	#
	# Why post-generation injection instead of new templates?
	#   - Zero template maintenance: IPv6 lines track IPv4 ports automatically.
	#   - No risk of breaking IPv4 servers: the block is entirely skipped when
	#     ipv6_addr is empty (no IPv6 on the server or domain).
	#   - Idempotent: guards check for [ipv6] presence before inserting.
	#
	# IP6 source priority:
	#   1. IP6= field in web.conf (set explicitly by v-change-web-domain-ipv6).
	#   2. First IPv6 address available to the user (get_user_ipv6 fallback).
	#      This auto-enables dual-stack on servers with IPv6 without any
	#      per-domain configuration step.
	#
	# nginx proxy injection:
	#   Appends "listen [ipv6]:port;" immediately after the matching IPv4
	#   listen line for both HTTP (port 80) and HTTPS (port 443).
	#
	# Apache backend injection:
	#   Expands "<VirtualHost ip:port>" to include "[ipv6]:port" so Apache
	#   binds both addresses from a single VirtualHost block — the standard
