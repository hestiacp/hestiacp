...
if [ -e "/etc/exim4/exim4.conf.template" ]; then
    ...
    sed -i 's|^smtp_banner = $smtp_active_hostname$|smtp_banner = "${smtp_active_hostname}\nsmtp_active_hostname = ${if exists {\/etc\/exim4\/ip_hostname.conf}{${lookup{$interface_address}lsearch{\/etc\/exim4\/ip_hostname.conf}{$value}{$primary_hostname}}}{$primary_hostname}}"|g' /etc/exim4/exim4.conf.template
fi
