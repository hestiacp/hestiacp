...
if [ -n "$(echo $DB_SYSTEM | grep -w mysql)" ]; then
	...
	version=$(mysql --defaults-file=/usr/local/devcp/conf/.mysql.localhost -e 'SELECT VERSION()')
	...
	if [ "$mysql_version2" = "10.6" ]; then
		...
		if [ -z "$test" ]; then
			...
			mysql --defaults-file=/usr/local/devcp/conf/.mysql.localhost -e "UPDATE mysql.global_priv SET priv=json_set(priv, '$.password_last_changed', UNIX_TIMESTAMP(), '$.plugin', 'mysql_native_password', '$.authentication_string', 'invalid', '$.auth_or', json_array(json_object(), json_object('plugin', 'unix_socket'))) WHERE User='root';"
		fi
	fi
fi
