...
# DevIT Control Panel upgrade script for target version 1.7.1
...
if ! (grep -q 'v-change-user-password' $DevIT/data/api/billing); then
	sed -i "s|v-make-tmp-file'|v-make-tmp-file,v-change-user-password'|g" $DevIT/data/api/billing
fi

# Apply update for path change of built-in IPset blacklist.sh
if [ "$FIREWALL_SYSTEM" = "iptables" ]; then
	rm -f $DevIT/data/firewall/ipset/blacklist.sh
	...
	if ($BIN/v-list-firewall-ipset plain | grep -q '/install/deb/firewall/ipset/blacklist.sh'); then
		...
		sed -i 's|/install/deb/firewall/ipset/blacklist.sh|/install/common/firewall/ipset/blacklist.sh|g' $DevIT/data/firewall/ipset.conf
	fi
fi
